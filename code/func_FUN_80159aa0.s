# metadata: {"startAddress": "0x80159aa0", "size": 984, "inst": 246, "name": "FUN_80159aa0", "endAddress": "0x80159e77"}

#include "def.h"

### Function: undefined FUN_80159aa0(void)
.global FUN_80159aa0
FUN_80159aa0:	# 0x80159aa0 - 0x80159e77
    stwu r1,-0x60(r1)	# stack
    mfspr r0,LR
    stw r0,0x64(r1)	# stack
    stw r31,0x5c(r1)	# stack
    stw r30,0x58(r1)	# stack
    mr r30,r4
    stw r29,0x54(r1)	# stack
    mr r29,r3
    stw r28,0x50(r1)	# stack
    lbz r0,0x12(r3)
    cmplwi r0,0x0
    beq LAB_80159e58
    lfs f1,-0x60f8(r2)	# = 255.0, op 1: FLOAT_804edcc8
    lfs f0,0x18(r29)
    fmuls f2,f1,f0
    psq_st f2,0x8(r1),0x1,GQR2_INDEX	# stack
    addi r3,r1,0xc
    lfs f1,-0x6108(r2)	# = 0.0, op 1: FLOAT_804edcb8
    lbz r31,0x8(r1)	# stack
    fmr f3,f1
    lfs f2,-0x60f4(r2)	# = 480.0, op 1: FLOAT_804edccc
    fmr f5,f1
    lfs f4,-0x60f0(r2)	# = 640.0, op 1: FLOAT_804edcd0
    lfs f6,-0x60ec(r2)	# = -30000.0, op 1: FLOAT_804edcd4
    bl __C_MTXOrtho	# void __C_MTXOrtho(double param_1, double param_2, double param_3, double param_4, double param_5, double param_6, float * param_7)
    li r0,0x1
    lis r3,-0x7fbc
    stw r0,0xd54(r30)
    addi r4,r3,0x2df0	# op 0: DAT_80442df0
    li r5,0x30
    lfs f0,0xc(r1)	# stack
    stfs f0,0xd58(r30)
    lfs f0,0x18(r1)	# stack
    stfs f0,0xd5c(r30)
    lfs f0,0x20(r1)	# stack
    stfs f0,0xd60(r30)
    lfs f0,0x28(r1)	# stack
    stfs f0,0xd64(r30)
    lfs f0,0x34(r1)	# stack
    stfs f0,0xd68(r30)
    lfs f0,0x38(r1)	# stack
    stfs f0,0xd6c(r30)
    lwz r28,0x19a0(r30)
    lwz r3,0x4(r28)
    addi r3,r3,0x4
    bl memcpy	# void * memcpy(void * __dest, void * __src, size_t __n)
    lwz r3,0x4(r28)
    lfs f1,-0x6108(r2)	# = 0.0, op 1: FLOAT_804edcb8
    lwz r0,0x0(r3)
    lfs f0,-0x60e8(r2)	# = 30000.0, op 1: FLOAT_804edcd8
    ori r0,r0,0x1
    stw r0,0x0(r3)
    stfs f1,0x1684(r30)
    stfs f0,0x1688(r30)
    lbz r0,0x16a7(r30)
    cmplwi r0,0x0
    beq LAB_80159bd8
    lbz r0,0x16a6(r30)
    cmplwi r0,0x0
    beq LAB_80159ba8
    lwz r0,0x170c(r30)
    ori r0,r0,0x100
    stw r0,0x170c(r30)
    lwz r0,0x17c4(r30)
    ori r0,r0,0x100
    stw r0,0x17c4(r30)
LAB_80159ba8:
    lwz r0,0x170c(r30)
    ori r0,r0,0x40
    stw r0,0x170c(r30)
    lwz r0,0x16a8(r30)
    ori r0,r0,0x40
    stw r0,0x16a8(r30)
    lwz r0,0x17c4(r30)
    ori r0,r0,0x40
    stw r0,0x17c4(r30)
    lwz r0,0x1760(r30)
    ori r0,r0,0x40
    stw r0,0x1760(r30)
LAB_80159bd8:
    lwz r0,0x16ac(r30)
    ori r0,r0,0x20
    stw r0,0x16ac(r30)
    lwz r0,0x16a8(r30)
    ori r0,r0,0x1
    stw r0,0x16a8(r30)
    lwz r0,0x1764(r30)
    ori r0,r0,0x20
    stw r0,0x1764(r30)
    lwz r0,0x1760(r30)
    ori r0,r0,0x1
    stw r0,0x1760(r30)
    lwz r0,0x1994(r30)
    cmpwi r0,0x1
    beq LAB_80159c30
    li r0,0x1
    stw r0,0x1994(r30)
    lwz r0,0x1994(r30)
    mulli r3,r0,0xbc
    addi r0,r3,0x1818
    add r0,r30,r0
    stw r0,0x1990(r30)
LAB_80159c30:
    li r6,0x1
    li r3,0x4
    stw r6,0x1644(r30)
    li r5,0x5
    li r4,0x0
    li r0,0x7
    stw r3,0x1648(r30)
    li r3,0x5
    stw r5,0x164c(r30)
    stw r5,0x1650(r30)
    lwz r5,0x170c(r30)
    ori r5,r5,0x1
    stw r5,0x170c(r30)
    lwz r5,0x16a8(r30)
    ori r5,r5,0x40
    stw r5,0x16a8(r30)
    lwz r5,0x17c4(r30)
    ori r5,r5,0x1
    stw r5,0x17c4(r30)
    lwz r5,0x1760(r30)
    ori r5,r5,0x40
    stw r5,0x1760(r30)
    stb r4,0x165c(r30)
    stw r0,0x1660(r30)
    lwz r0,0x170c(r30)
    ori r0,r0,0x8
    stw r0,0x170c(r30)
    lwz r0,0x16a8(r30)
    ori r0,r0,0x40
    stw r0,0x16a8(r30)
    lwz r0,0x17c4(r30)
    ori r0,r0,0x8
    stw r0,0x17c4(r30)
    lwz r0,0x1760(r30)
    ori r0,r0,0x40
    stw r0,0x1760(r30)
    stb r6,0x1656(r30)
    stb r6,0x1657(r30)
    stb r4,0x1658(r30)
    lwz r0,0x170c(r30)
    ori r0,r0,0x4
    stw r0,0x170c(r30)
    lwz r0,0x16a8(r30)
    ori r0,r0,0x40
    stw r0,0x16a8(r30)
    lwz r0,0x17c4(r30)
    ori r0,r0,0x4
    stw r0,0x17c4(r30)
    lwz r0,0x1760(r30)
    ori r0,r0,0x40
    stw r0,0x1760(r30)
    stw r4,0xd8c(r30)
    lwz r0,0x16b8(r30)
    ori r0,r0,0x2
    stw r0,0x16b8(r30)
    lwz r0,0x16a8(r30)
    ori r0,r0,0x2
    stw r0,0x16a8(r30)
    lwz r0,0x1770(r30)
    ori r0,r0,0x2
    stw r0,0x1770(r30)
    lwz r0,0x1760(r30)
    ori r0,r0,0x2
    stw r0,0x1760(r30)
    bl GetCommonDesireData
    mr r4,r3
    mr r3,r30
    bl cFielder_X_SetAction
    lwz r3,0xc(r29)
    lwz r0,0x1740(r30)
    cmplw r0,r3
    bne LAB_80159d68
    lwz r0,0x17f8(r30)
    cmplw r0,r3
    bne LAB_80159d68
    lbz r0,0x7(r3)
    cmplwi r0,0x0
    beq LAB_80159d88
LAB_80159d68:
    stw r3,0x1740(r30)
    lwz r0,0x173c(r30)
    ori r0,r0,0x1
    stw r0,0x173c(r30)
    stw r3,0x17f8(r30)
    lwz r0,0x17f4(r30)
    ori r0,r0,0x1
    stw r0,0x17f4(r30)
LAB_80159d88:
    li r0,0x98
    mr r3,r30
    stw r0,0x1998(r30)
    li r4,0x4
    bl FUN_802b706c
    lfs f3,-0x6108(r2)	# = 0.0, op 1: FLOAT_804edcb8
    lis r4,-0x33ff
    li r0,0xff
    lfs f2,-0x60f0(r2)	# = 640.0, op 1: FLOAT_804edcd0
    stfs f3,-0x8000(r4)	# op 1: DAT_cc008000
    mr r3,r30
    lfs f1,-0x6104(r2)	# = 1.0, op 1: FLOAT_804edcbc
    stfs f3,-0x8000(r4)	# op 1: DAT_cc008000
    lfs f0,-0x60f4(r2)	# = 480.0, op 1: FLOAT_804edccc
    stfs f3,-0x8000(r4)	# op 1: DAT_cc008000
    stb r0,-0x8000(r4)	# op 1: DAT_cc008000
    stb r0,-0x8000(r4)	# op 1: DAT_cc008000
    stb r0,-0x8000(r4)	# op 1: DAT_cc008000
    stb r31,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f3,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f3,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f2,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f3,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f3,-0x8000(r4)	# op 1: DAT_cc008000
    stb r0,-0x8000(r4)	# op 1: DAT_cc008000
    stb r0,-0x8000(r4)	# op 1: DAT_cc008000
    stb r0,-0x8000(r4)	# op 1: DAT_cc008000
    stb r31,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f1,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f3,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f3,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f0,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f3,-0x8000(r4)	# op 1: DAT_cc008000
    stb r0,-0x8000(r4)	# op 1: DAT_cc008000
    stb r0,-0x8000(r4)	# op 1: DAT_cc008000
    stb r0,-0x8000(r4)	# op 1: DAT_cc008000
    stb r31,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f3,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f1,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f2,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f0,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f3,-0x8000(r4)	# op 1: DAT_cc008000
    stb r0,-0x8000(r4)	# op 1: DAT_cc008000
    stb r0,-0x8000(r4)	# op 1: DAT_cc008000
    stb r0,-0x8000(r4)	# op 1: DAT_cc008000
    stb r31,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f1,-0x8000(r4)	# op 1: DAT_cc008000
    stfs f1,-0x8000(r4)	# op 1: DAT_cc008000
    bl FUN_802b7060
    mr r3,r30
    bl FUN_802b5cec
    sth r3,0x10(r29)
LAB_80159e58:
    lwz r0,0x64(r1)	# stack
    lwz r31,0x5c(r1)	# stack
    lwz r30,0x58(r1)	# stack
    lwz r29,0x54(r1)	# stack
    lwz r28,0x50(r1)	# stack
    mtspr LR,r0
    addi r1,r1,0x60
    blr
